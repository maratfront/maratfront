### Frontend Developer | Vue & Nuxt Specialist

📍 **Location:** Yaroslavl, Russia  
📅 **Age:** 23 (Born: July 18, 2002)  
🎓 **Education:** Bachelor's in Software Engineering, YSTU (2021-2025)  
💼 **Experience:** 2 years as Frontend Developer (Nuxt)

---

## 🛠 Tech Stack

### **Core Technologies**
![HTML5](https://img.shields.io/badge/-HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/-CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)

### **Vue Ecosystem**
![Vue.js](https://img.shields.io/badge/-Vue.js-4FC08D?style=flat&logo=vuedotjs&logoColor=white)
![Vue Router](https://img.shields.io/badge/-Vue_Router-4FC08D?style=flat&logo=vuerouter&logoColor=white)
![Pinia](https://img.shields.io/badge/-Pinia-FFD02F?style=flat&logo=pinia&logoColor=black)
![Vuex](https://img.shields.io/badge/-Vuex-4FC08D?style=flat&logo=vuex&logoColor=white)

### **Frameworks & Tools**
![Nuxt](https://img.shields.io/badge/-Nuxt-00DC82?style=flat&logo=nuxtdotjs&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/-Tailwind_CSS-06B6D4?style=flat&logo=tailwindcss&logoColor=white)

### **Testing**
![Vitest](https://img.shields.io/badge/-Vitest-6E9F18?style=flat&logo=vitest&logoColor=white)
![Testing Library](https://img.shields.io/badge/-Testing_Library-E33332?style=flat&logo=testinglibrary&logoColor=white)

---

## 💼 Work Experience

### **Frontend Developer** ZooMir *(Remote, Stavropol)*
*Duration:* 2 years *(2023 - Present)*

- Developed and maintained web applications using **Nuxt.js** framework
- Implemented responsive and interactive UI components with **Vue.js**
- Managed state using **Pinia** state management libraries
- Created comprehensive test suites using **Vitest** and **Testing Library**
- Implemented unit, component, and integration tests for Vue components
- Set up CI/CD pipelines with automated testing
- Collaborated with backend teams to integrate RESTful APIs
- Optimized application performance and improved user experience
- Participated in code reviews and followed best practices

---

## 🎓 Education

### **Yaroslavl State Technical University (YSTU)**
**Bachelor of Software Engineering** *(2021 - 2025)*

- Specialized in software development methodologies
- Completed projects in web development and software architecture
- Gained foundation in algorithms, data structures, and system design
- Software testing and quality assurance coursework

---

## 🚀 Current Focus

- Mastering advanced **Nuxt 3** features
- Exploring **Vue 3 Composition API** patterns
- Improving **TypeScript** skills for type-safe applications

---

*⭐️ Feel free to explore my repositories and contribute to any projects that interest you!*
